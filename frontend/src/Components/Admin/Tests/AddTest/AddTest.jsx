...
function ExamForm2025({ onClose, onSubmit, initialData, isStandalone = false }) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            let response, data;
            if (id) {
                // Chỉnh sửa đề thi (PUT)
                response = await fetch(`http://localhost:5000/api/tests/${id}`, {
                    method: 'PUT',
                    headers: {
                ...
                // Tạo mới đề thi (POST)
                response = await fetch('http://localhost:5000/api/tests', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json'
                    },
            ...
            if (response.ok && data.success) {
                ...
                setError(data.message || `Thao tác ${id ? 'cập nhật' : 'thêm'} đề thi thất bại`);
                setSubmitLoading(false);
            }
        } catch (err) {
        ...
        <div className={containerClass}>
            <div className="exam2025-form-container">
                <div className="exam2025-form-header">
                    <h2>{id ? 'Chỉnh sửa đề thi' : 'Tạo đề thi'}</h2>
                    {isStandalone && (
                        ...
                        <div className="form-section">
                            ...
                            <div className="form-group">
                                ...
                                <select name="subject" value={form.subject} onChange={handleChange} required>
                                    ...
                                    <option value="GDCD">Tin học</option>
                                </select>
                            </div>
                        </div>
